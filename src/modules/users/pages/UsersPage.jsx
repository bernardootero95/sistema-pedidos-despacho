...
export const UsersPage = () => {
  ...
  const handleFormSuccess = () => {
    setIsFormOpen(false);
    setUserToEdit(null);
    cargarDatos(); // Refrescamos la tabla para ver los cambios
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6 relative">
      {/* MODAL DE CREACIÓN / EDICIÓN DE CORREO */}
      {isFormOpen && (
        <UserForm
          roles={roles}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <button
          ...
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-3 sm:py-2.5 bg-primary hover:bg-primary-hover active:scale-95 text-white text-sm font-semibold rounded-xl sm:rounded-lg shadow-sm transition-all min-h-11"
        >
              ...
              <div className="flex items-center gap-1.5 text-[11px] font-semibold">
                {user.correo ? (
                  <span className="flex items-center gap-1 text-emerald-600">
                    <Mail className="w-3.5 h-3.5" /> Correo configurado
                  </span>
                ) : (
                  <span className="flex items-center gap-1 text-slate-400">
                    <MailX className="w-3.5 h-3.5" /> Sin correo
                  </span>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    title="Editar correo de recuperación"
                  ...
                  <td className="py-4 px-6">
                    ...
                    {user.correo ? (
                      <p className="mt-1 text-[10px] font-semibold text-emerald-600 flex items-center gap-1">
                        <Mail className="w-3 h-3" /> Correo configurado
                      </p>
                    ) : (
                      <p className="mt-1 text-[10px] font-semibold text-slate-400 flex items-center gap-1">
                        <MailX className="w-3 h-3" /> Sin correo
                      </p>
                    )}
                  </td>
                  <td className="py-4 px-6">
