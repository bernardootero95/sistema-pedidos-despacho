-- ============================================================================
-- Precio a crédito: tercer tipo de precio especial, igual que "frío"
-- ============================================================================
-- Mismo mecanismo que precio_frio: precio alterno único por producto
-- (no por franjas), opcional, para productos vendidos a crédito a un
-- valor mayor. Mismos roles que frío: soporte/gerencia/despachador,
-- nunca vendedor. Se activa manualmente por línea (no depende de
-- cantidad, a diferencia del mayorista).
-- ============================================================================

ALTER TABLE productos ADD COLUMN precio_credito NUMERIC(12,2);

ALTER TABLE productos
  ADD CONSTRAINT chk_productos_precio_credito CHECK (precio_credito IS NULL OR precio_credito >= 0);
...
ALTER TABLE pedidos_detalle
  ...
  CHECK (tipo_precio IN ('normal', 'mayorista', 'frio', 'credito'));

-- CREATE OR REPLACE no cambia la firma: al agregar p_precio_credito
-- quedaría un overload viejo de 6 argumentos huérfano si no se borra.
DROP FUNCTION IF EXISTS resolver_precio_pedido(UUID, TEXT, NUMERIC, NUMERIC, INTEGER, TEXT);
...
REVOKE ALL ON FUNCTION resolver_precio_pedido(UUID, TEXT, NUMERIC, NUMERIC, INTEGER, TEXT, NUMERIC) FROM PUBLIC, authenticated;
